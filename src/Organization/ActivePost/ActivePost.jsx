...
function ActivePost(props) {
  ...
  const handleEdit = (item) => {
    ...
    if (item.category == "scrims") navigate(`/updatescrim/${item.id}`);
    if (item.category == "dailymatch") navigate(`/updatedailymatch/${item.id}`);
    if (item.category == "openrooms") navigate(`/updateopenroom/${item.id}`);
  };
  ...
  const handleDelete = async (item) => {
    ...
    if (isConfirmed) {
      try {
        const res = await del(
          `http://localhost:4000/${item.category}/${item.id}`
        );
  ...
  function slotlist(item) {
    ...
    if (item.category == "scrims") item.url = "/UserScrimPlayerRegisterForm";
