...
import { newUser } from "../api-Helpers/api-helpers";
  ...
  return (
    <div className="maindiv-otp">
      <div>
        ...
        ></OTPInput>
        {OtpError ? (
          <span className={classes.text_color_error}>Enter Valid OTP</span>
        ) : OtpValid ? (
          <span className={classes.text_color_error}>Enter OTP</span>
        ) : null}
        <div className={`${classes.loginBtn} subdiv-verify`}>
          <button
            onClick={() => VerifierOtp()}
            className="w-full flex gap-1 items-center justify-center py-2.5 text-orange rounded  "
          >
            <span> Verify</span>
          </button>
        </div>
